...
class EmployeeReviews < Minitest::Test
  ...
  def test_total_department_salary
    ...
    assert marketing.add_employee(new_employee)
    assert marketing.add_employee(old_employee)
    assert_equal 90000.00, marketing.department_salary
  end
  ...
  def test_add_employee_review
    ...
    updated_xavier = Employee.find(xavier.id)
    assert_equal updated_xavier.review, xavier.review
    xavier.destroy
    assert_raises ActiveRecord::RecordNotFound do
      Employee.find(xavier.id)
    end
  end
  ...
  def test_department_raises_based_on_criteria
    ...
    marketing.add_employee(xavier)
    marketing.add_employee(new_employee)
    marketing.add_employee(old_employee)
    xavier.set_employee_performance(true)
    new_employee.set_employee_performance(true)
    old_employee.set_employee_performance(false)
    marketing.department_raise(14000.00) {|e| e.satisfactory == true && e.salary < 60000.00}
  ...
  def test_department_must_have_name
    blank = Department.new()
    refute blank.save
  end

  def test_employees_must_be_paid
    blank = Employee.new
    binding.pry
    refute blank.save
  end
